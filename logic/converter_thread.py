# converter_thread.py - Worker thread for markdown conversion
# Uses pypandoc_binary for Word, pandas+openpyxl+pytablewriter for Excel
...
class ConversionWorker(QThread):
    ...
    def __init__(self, input_file, output_file, conversion_type, use_syntax_highlighting=True):
        super().__init__()
    ...
    def convert_to_word(self):
        """Convert to Word using pypandoc_binary"""
        from core.logger import Logger
        logger = Logger()
        
        import pypandoc
        try:
            logger.info(f"Starting Word conversion: {self.input_file}")
            ...
            status = "with syntax highlighting" if self.use_syntax_highlighting else ""
            logger.info(f"Word conversion successful: {self.output_file}")
    ...
    def convert_to_excel(self):
        """Convert to Excel using pandas + markdown + BeautifulSoup + openpyxl + pytablewriter"""
        from core.logger import Logger
        logger = Logger()
        ...
        try:
            ...
            logger.warning("pytablewriter not available")
        
        try:
            logger.info(f"Starting Excel conversion: {self.input_file}")
            ...
            if tables:
                ...
                self.progress.emit(100)
                logger.info(f"Excel conversion successful: {len(tables)} tables to {self.output_file}")
            ...
            if PYTABLEWRITER_AVAILABLE:
                ...
                logger.info("No tables found, trying pytablewriter")
                ...
                if data:
                    ...
                    self.progress.emit(100)
                    logger.info(f"Pytablewriter conversion successful: {self.output_file}")
                    self.finished.emit(True, f"Converted structured data to Excel using pytablewriter:\n{self.output_file}")
                    return

            logger.warning("No tables or structured data found")
